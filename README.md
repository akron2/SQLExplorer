...
- LOB (Oracle CLOB/NCLOB/BLOB/BFILE, PostgreSQL text/bytea): маркеры в сетке, просмотр 8 KiB → 1 MiB, hex и текст с выбором кодировки, потоковое сохранение в файл с прогрессом и отменой.
- Бюджет памяти PostgreSQL для больших значений с подтверждением дополнительного бюджета и честными пометками.
- Экспорт результатов в Excel (.xlsx): режимы LOB «маркеры», «частично» и «каждый LOB отдельным файлом с гиперссылкой», догрузка оставшихся строк по запросу; CSV сохранён отдельно.
