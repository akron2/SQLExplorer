...
- ESLint и строгий TypeScript проходят; 139 модульных тестов в 12 файлах, включая каталог, анализатор контекста, completion и настройки интерфейса.
- Browser E2E: 8 сценариев, включая каталог схем, поиск объектов, смену текущей схемы и масштаб интерфейса.
- Electron E2E: 7 сценариев на живых БД, включая показ объектов автодополнения после подключения.
...
Найденный при приёмке дефект пустых подсказок (несовпадение IPC-контракта `catalog:complete`) исправлен; Electron E2E проверяет автодополнение на живом Oracle. Быстрый перезапуск приложения защищён `PRAGMA busy_timeout = 5000`.
