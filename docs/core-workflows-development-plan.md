...
Этапы этого среза уже отражены в [статусе реализации](implementation-status.md). Вся новая и оставшаяся работа ведётся только в [едином бэклоге](backlog.md).
...
Демонстрационный каталог используется только браузерным mock и тестовыми fixtures. Production bootstrap не передаёт метаданные в renderer: каталог живёт в SQLite и отдаёт схемы и объекты постранично. Выбор профиля сам по себе не имитирует подключение.
