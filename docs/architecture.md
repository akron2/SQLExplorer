...
Дата: 2026-09-09. Версия документа: 0.1. Статус: базовая архитектура MVP, до реализации.

Документ фиксирует требования из обсуждения и рабочие проектные решения. Движок автодополнения и совместимость с конкретными версиями серверов остаются открытыми. Подробности оценки движков приведены в [обзоре автодополнения](autocompletion.md), проверки отзывчивости — в [плане измерений](performance.md).
...
| Локальное хранение | SQLite | История, настройки, профили и кэш метаданных |
| Автодополнение | Заменяемый провайдер на диалект | Первый кандидат — адаптация Bytebase в отдельном Go-процессе; выбор зависит от проверки |
...
flowchart TD
    ...
    Main --> DB["DB-worker: сессии и выполнение"]
    DB --> OA["Адаптер Oracle"]
    DB --> PA["Адаптер PostgreSQL"]
    OA --> Oracle[(Oracle)]
    ...
    DB --> Metadata["Кэш метаданных по контексту соединения"]
    Metadata --> Provider
...
Это схема ответственности; каждому блоку не обязательно соответствует отдельный процесс. В MVP предлагается один DB-worker с двумя адаптерами и отдельное выполнение анализа языка. Worker не создаётся на каждую вкладку. Конкретное размещение фонового локального хранилища выбирается при реализации без синхронного дискового I/O в renderer и main.

Renderer отвечает за видимую рабочую область. Main управляет жизненным циклом окна, системными действиями и IPC. DB-worker владеет соединениями, транзакциями и курсорами. Провайдер языка получает текст и сведения о схеме, а не выполняет пользовательский SQL.
...
| MetadataSnapshot | Версия сведений об объектах, доступных в конкретном контексте |
...
Общий интерфейс провайдера скрывает выбор движка. Запрос содержит диалект, версию документа, позицию и контекст соединения. Устаревший ответ отбрасывается. Базовые подсказки используют кэш, обновляемый в фоне. Обработка большого SQL ограничена по времени и не блокирует ввод. Детали и источники — в [autocompletion.md](autocompletion.md).
...
| Единый DB-worker | Потеря его сессий при аварии; возможная последующая изоляция по профилям |
...
2. Оценить готовые движки автодополнения на общем корпусе для Oracle и PostgreSQL и зафиксировать выбор в документах.
...
5. Подключить дерево объектов, кэш метаданных и провайдеры подсказок.
